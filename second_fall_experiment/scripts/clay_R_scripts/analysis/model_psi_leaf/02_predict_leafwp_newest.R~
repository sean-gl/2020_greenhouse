...
saveRDS(comb_all_predict, '/home/wmsru/Documents/Clay/greenhouse_2019/second_fall_experiment/analysis/clay/output_data/combined_data_predicted_psi_leaf.rds')
...
tuneGrid = expand.grid(mtry = seq(1, 20, 2), 
                       splitrule = 'extratrees', #c('variance', 'extratrees', 'maxstat'),
                       min.node.size = 1)
...
### optional: try using "quick" forest to predict
# randomForest_pl <- rf_quick
# train_pred <- predict(randomForest_pl, df2_train_scaled)$predictions # on scaled data
# test_pred <- predict(randomForest_pl, df2_test_scaled)$predictions
...
# plot 2nd treatments
sub <- subset(comb_all_predict, date(by15) >= '2019-11-04' & date(by15) <= '2019-11-12', drop = F)
# plot 3rd treatments
sub <- subset(comb_all_predict, date(by15) >= '2019-12-01' & date(by15) <= '2019-12-08')
