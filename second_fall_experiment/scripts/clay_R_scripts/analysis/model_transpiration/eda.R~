...
ind <- dat$mean_T_mL_hr < 0 & !is.na(dat$mean_T_mL_hr)
nrow(dat[ind, ]) / nrow(dat)
dat$mean_T_mL_hr[ind] <- 0
...
ggplot(sub, aes(x=altd, y=mean_T_mL_hr, color=treatment)) + geom_point() + geom_smooth(method = 'lm', formula = y ~ poly(x,1))
ggplot(sub, aes(x=leaftemp_mean, y=mean_T_mL_hr, color=treatment)) + geom_point() + geom_smooth(method = 'lm', formula = y ~ poly(x,2))

m=lm(mean_T_mL_hr ~ poly(altd,1) + mean_irrig_kg, dat[!is.na(dat$altd),]); summary(m)
m=lm(mean_T_mL_hr ~ poly(altd,2) * mean_irrig_kg, dat[!is.na(dat$altd),]); summary(m)

m=lm(mean_T_mL_hr ~ poly(leaftemp_mean,2) * mean_irrig_kg, dat[!is.na(dat$altd),]); summary(m)
...
m=lm(mean_T_mL_hr ~ poly(altd,2)*mean_irrig_kg, dat[!is.na(dat$altd),]); summary(m)
m=lm(mean_T_mL_hr ~ poly(VPD_air,2)*mean_irrig_kg, dat[!is.na(dat$altd),]); summary(m)

m=lm(mean_T_mL_hr ~ poly(altd,2)*treatment, dat[!is.na(dat$altd),]); summary(m)

# add leaftemp_mean, r2=0.74
m=lm(mean_T_mL_hr ~ poly(altd,1)*mean_irrig_kg + leaftemp_mean, dat[!is.na(dat$altd),]); summary(m)
m=lm(mean_T_mL_hr ~ poly(altd,2)*pleaftemp_mean, dat[!is.na(dat$altd),]); summary(m)

m=lm(mean_T_mL_hr ~ poly(altd,2)*mean_irrig_kg + leaftemp_mean +line_PAR_east_umol_m2_s, dat[!is.na(dat$altd),]); summary(m)
m=lm(mean_T_mL_hr ~ leaftemp_mean*line_PAR_east_umol_m2_s, dat[!is.na(dat$altd),]); summary(m)
m=lm(mean_T_mL_hr ~ altd*line_PAR_east_umol_m2_s, dat[!is.na(dat$altd),]); summary(m)

# 2-way interactions all significant....r2=0.76
m=lm(mean_T_mL_hr ~ poly(leaftemp_mean,2)*mean_irrig_kg + line_PAR_west_umol_m2_s, dat[!is.na(dat$altd),]); summary(m)
# not as good?
m=lm(mean_T_mL_hr ~ poly(altd,2)*mean_irrig_kg + line_PAR_west_umol_m2_s, dat[!is.na(dat$altd),]); summary(m)
...
plot(dat$leaftemp_mean, dat$mean_T_mL_hr, use = 'complete.obs')
plot(dat$leaftemp_highest_avail, dat$mean_T_mL_hr, use = 'complete.obs')
plot(dat$leaftemp_top, dat$mean_T_mL_hr, use = 'complete.obs', col=dat$treatment)


cor(dat$altd, dat$mean_T_mL_hr, use = 'complete.obs')
cor(dat$leaftemp_top, dat$mean_T_mL_hr, use = 'complete.obs')
...
predictors <- names(dat)[!names(dat) %in% c('by15','date','block','treatment','mean_T_mL_hr')]
out <- lapply(predictors, function(p) {
  # print(p)
  tryCatch({
    return(c(p, cor(sub[[p]], sub$mean_T_mL_hr, use = 'complete.obs')))
  }, error = function(e) {
    print(e); return(NA)
  })
})
...
plot(dat$by15, dat$mean_T_mL_hr, type = 'l', col = 'blue')
...
cor(sub$par1_n, sub$mean_T_mL_hr, use = 'complete.obs')
plot(sub$par1_n, sub$mean_T_mL_hr, col = sub$treatment)
plot(sub$leaftemp_mean, sub$mean_T_mL_hr, col = sub$treatment)
plot(sub$leaftemp_mean, sub$par1_n, col = sub$treatment)

# PAR vs. T
# all data: looks like quadratic
ggplot(sub, aes(x=line_PAR_east_umol_m2_s, y=mean_T_mL_hr, color=treatment)) + geom_point() + geom_smooth(method = 'lm', formula = y ~ poly(x,2))
...
lines(mean_T_mL_hr*5~by15, type='l', col='red', data=sub2)
...
y = ddply(sub2, .(date), function(x) {
  ...
  mt = which(x$mean_T_mL_hr == max(x$mean_T_mL_hr, na.rm = T))
...
summary(lm(mean_T_mL_hr ~ poly(line_PAR_east_umol_m2_s, 2) + treatment, 
           data = subset(sub, !is.na(mean_T_mL_hr) & !is.na(line_PAR_east_umol_m2_s))))

summary(lm(mean_T_mL_hr ~ poly(par1_n, 2) + treatment, 
           data = subset(sub, !is.na(mean_T_mL_hr) & !is.na(par1_n))))

summary(lm(mean_T_mL_hr ~ poly(sht1_high_rh, 1) + poly(par1_n, 2) + treatment, 
           data = sub[complete.cases(sub[,c('sht1_high_rh','par1_n','mean_T_mL_hr')]),]))

summary(lm(mean_T_mL_hr ~ poly(sht1_high_rh, 1) + poly(line_PAR_east_umol_m2_s, 2) + treatment + block +
             leaftemp_mean + minutes, 
           data = sub[complete.cases(sub[,c('sht1_high_rh','line_PAR_east_umol_m2_s','mean_T_mL_hr')]),]))

x=diff(sub$par1_n)
y=diff(sub$mean_T_mL_hr)
plot(x,y)
cor(x, y, use = 'complete.obs')

ggplot(sub, aes(x=by15)) +
  geom_line(aes(y=par1_n, color='red')) +
  geom_line(aes(y=mean_T_mL_hr*100, color='blue'))


## ------ look  at T vs. psi_leaf
# measured
ggplot(sub, aes(x=mean_psi_MPa, y=mean_T_mL_hr, color=treatment)) + geom_point() + geom_smooth(method = 'lm', formula = y ~ poly(x,2))
summary(lm(mean_T_mL_hr~poly(mean_psi_MPa,2) + treatment, dat[!is.na(dat$mean_psi_MPa),]))

# modeled
ggplot(sub, aes(x=predicted_psi_leaf, y=mean_T_mL_hr, color=treatment)) + geom_point() #+ geom_smooth(method = 'lm')
summary(lm(mean_T_mL_hr~poly(predicted_psi_leaf,2) + treatment, dat[!is.na(dat$predicted_psi_leaf),]))
...
lines(mean_T_mL_hr*5~by15, type='l', col='blue', data=sub2)
...
sub2 <- subset(sub, block == 'M' &date >= '2019-10-10' & date <='2019-11-06')
plot(mean_T_mL_hr~by15, type='l',data=sub2, main='moderate trt',col='blue')
